...
using impl_t = std::function<bool(Server &, identifier_t, json &)>;

ServerHandlers::ServerHandlers() : Handlers() {
    impl_t handler_signin = [](Server &server, identifier_t id, json &request) -> bool {
        auto &&login = request[parts::LOGIN];
        auto &&password = request[parts::PASSWORD];
        server.do_signin(id, login, password);
        return false;
    };
    impl_t handler_signout = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_signout(id);
        return false;
    };
    impl_t join = [](Server &server, identifier_t id, json &request) -> bool {
        auto &&name = request[parts::NAME];
        auto &&password = request[parts::PASSWORD];
        server.do_join(id, name, password);
        return false;
    };
    impl_t create = [](Server &server, identifier_t id, json &request) -> bool {
        auto &&name = request[parts::NAME];
        auto &&password = request[parts::PASSWORD];
        server.do_create(id, name, password);
        return false;
    };
    impl_t leave = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_leave(id);
        return false;
    };
    impl_t tables = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_tables(id);
        return false;
    };
    impl_t users = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_users(id);
        return false;
    };
    impl_t write = [](Server &server, identifier_t id, json &request) -> bool {
    ...
    impl_t disconnect = [](Server &server, identifier_t id, json &request) -> bool {
        return true;
    };
    impl_t sync = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_sync(id);
        return false;
    };
    impl_t handler_signup = [](Server &server, identifier_t id, json &request) -> bool {
        auto &&login = request[parts::LOGIN];
        auto &&password = request[parts::PASSWORD];
        server.do_signup(id, login, password);
        return false;
    };
    impl_t set_permission = [](Server &server, identifier_t id, json &request) -> bool {
        auto &&login = request[parts::LOGIN];
        auto &&permission = request[parts::PERMITION];
        server.do_set_permission(id, login, permission);
        return false;
    };
    impl_t command_spin = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_spin(id);
        return false;
    };
    impl_t command_bet = [](Server &server, identifier_t id, json &request) -> bool {
    ...
    impl_t command_bets = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_bets(id);
        return false;
    };
    impl_t command_balance = [](Server &server, identifier_t id, json &request) -> bool {
        server.do_balance(id);
        return false;
    };
    impl_t command_kick = [](Server &server, identifier_t id, json &request) -> bool {
